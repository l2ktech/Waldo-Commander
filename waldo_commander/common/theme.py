...
/* Live joint range HUD above the 3D scene. It shares the same angle stream and
   soft-limit source as Joint Jog, so the model/readout/range cannot drift. */
.joint-range-hud {
  position: absolute;
  top: 12px;
  left: 50%;
  ...
  gap: 5px !important;
  pointer-events: none;
  background: color-mix(in srgb, var(--q-dark-page, #16191d) 86%, transparent) !important;
  backdrop-filter: blur(8px);
}

.joint-range-hud-title {
  min-height: 18px;
  gap: 5px;
  color: var(--ctk-text);
  font-size: 12px;
...
.joint-range-item {
  min-width: 0;
  padding: 5px 6px;
  border: 1px solid color-mix(in srgb, var(--ui-tint) 24%, transparent);
  border-radius: 7px;
  background: color-mix(in srgb, var(--q-dark-page, #16191d) 66%, transparent);
}

.joint-range-heading { min-height: 17px; }
.joint-range-name { color: var(--ui-tint); font-size: 11px; font-weight: 700; }
.joint-range-current { color: var(--ctk-text); font-size: 12px; font-weight: 700; }
.joint-range-progress { height: 5px; margin: 2px 0 3px; }
.joint-range-limits,
...
.joint-range-remaining { color: var(--ctk-text-muted, var(--ctk-muted)); }
...
@media (max-width: 1250px) {
  ...
  .joint-range-grid { grid-template-columns: repeat(3, minmax(0, 1fr)); }
  .joint-range-hud-subtitle { display: none; }
}
